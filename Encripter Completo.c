...
//funciona perfeitamente
unsigned char gf_mult(unsigned char h1, unsigned char h2) {
  int r;
  if (h1 == 0x01) {
    return h2;
  }
...
void MixColumns(unsigned char M[4][4]) {
  ...
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
      for (int k = 0; k < 4; k++) {
        temp[i][j] ^= (gf_mult(  matriz[i][k], M[k][j]));
      }
...
int main() {
  ...
  char texto[10000]; // Pode ajustar o tamanho conforme sua necessidade
  char chave[17];    // A chave pode ter até 16 caracteres + 1 para o caractere nulo
  char RKey [4][4];

  // Leitura do texto
  printf("Digite o texto (ate 10000 caracteres):\n");
  fgets(texto, sizeof(texto), stdin);

  // Remova a quebra de linha do texto, se houver
  ...
  // Leitura da chave
  printf("Digite a chave (ate 16 caracteres):\n");
  fgets(chave, sizeof(chave), stdin);

  // Remova a quebra de linha da chave, se houver
...
printf("Matriz Mensagem encriptada:\n");
for (int z = 0; z < q_M; z++) {
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
